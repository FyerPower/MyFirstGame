...
LRESULT CALLBACK windows_callback(HWND window, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg) {
        ...
        case WM_SIZE: {
            ...
            PLOG_INFO << "Window Resized.. x(" << input->screenSize.x << ") y(" << input->screenSize.y << ")";
            break;
        }
        ...
        case WM_SYSKEYUP: {
            ...
            PLOG_INFO << "Key Event Triggered.. keyCode(" << keyCode << ") isDown(" << isDown << ")";
            break;
        }
        ...
        case WM_MBUTTONUP: {
            ...
            int mouseCode = (msg == WM_LBUTTONDOWN || msg == WM_LBUTTONUP)   ? VK_LBUTTON
                            : (msg == WM_MBUTTONDOWN || msg == WM_MBUTTONUP) ? VK_MBUTTON
                                                                             : VK_RBUTTON;
            ...
            PLOG_INFO << "Mouse Event Triggered.. keyCode(" << keyCode << ") isDown(" << isDown << ")";
            break;
        }
...
bool platform_create_window(int width, int height, const char* title)
{
    ...
    // Fake Window initializing OpenGL
    {
        ...
        if (window == NULL) {
            PLOG_ASSERT(false, "Failed to create Windows Window");
            return false;
        }

        HDC fakeDC = GetDC(window);
        if (!fakeDC) {
            PLOG_ASSERT(false, "Failed to get HDC");
            return false;
        }
        ...
        if (!pixelFormat) {
            PLOG_ASSERT(false, "Failed to choose pixel Format");
            return false;
        }

        //
        if (!SetPixelFormat(fakeDC, pixelFormat, &pfd)) {
            PLOG_ASSERT(false, "Failed to set pixel format");
            return false;
        }
        ...
        if (!fakeRC) {
            PLOG_ASSERT(false, "Failed to create Render context");
            return false;
        }

        if (!wglMakeCurrent(fakeDC, fakeRC)) {
            PLOG_ASSERT(false, "Failed to make current");
            return false;
        }
        ...
        if (!wglCreateContextAttribsARB || !wglChoosePixelFormatARB) {
            PLOG_ASSERT(false, "Failed to load OpenGL functions");
            return false;
        }
    ...
    // Actual OpenGL initialization
    {
        ...
        dc = GetDC(window);
        if (!dc) {
            PLOG_ASSERT(false, "Failed to get DC");
            return false;
        }
        ...
        if (!wglChoosePixelFormatARB(dc, pixelAttribs,
            ...
            PLOG_ASSERT(0, "Failed to wglChoosePixelFormatARB");
            return false;
        }
        ...
        if (!SetPixelFormat(dc, pixelFormat, &pfd)) {
            PLOG_ASSERT(0, "Failed to SetPixelFormat");
            return true;
        }
        ...
        if (!rc) {
            PLOG_ASSERT(0, "Failed to crate Render Context for OpenGL");
            return false;
        }

        if (!wglMakeCurrent(dc, rc)) {
            PLOG_ASSERT(0, "Failed to wglMakeCurrent");
            return false;
        }
    }
...
void* platform_load_gl_function(const char* funcName)
{
    ...
    if (!proc) {
        // If for whatever reason it fails, lets fallback to loading it via the opengl32.dll which most all computers
        // should have
        ...
        if (!proc) {
            PLOG_ASSERT(proc, "Failed to load gl function: glCreateProgram");
            return nullptr;
        }
    }
    return (void*)proc;
...
void* platform_load_dynamic_library(const char* dll)
{
    ...
    PLOG_ASSERT(result, std::string("Failed to load dll: ") + dll);
    return result;
}
...
void* platform_load_dynamic_function(void* dll, const char* funName)
{
    ...
    PLOG_ASSERT(proc, std::string("Failed to load function from DLL: ") + funName);
    return (void*)proc;
}
...
bool platform_free_dynamic_library(void* dll)
{
    ...
    PLOG_ASSERT(freeResult, "Failed to FreeLibrary");
